...
<title>2A400 bacpipe embedding visualisations</title>
<style>
...
<h1>2A400 bacpipe embedding visualisations</h1>
<p class="muted">Date: {html.escape(manifest["date"])} · generated from existing <code>.npy</code> embeddings; no model inference was performed.</p>
...
def build_report(
    *,
    ...
    for model in models:
        ...
        if len(X) == 0:
            print("  skipped: no embeddings for requested date")
            continue
